...
class tabbedWindow(QWidget):
	...
	def update_wedgePlot(self):
		...
		self._update_earthmodel_ax.imshow(self.earthmod, cmap='viridis_r', aspect=0.2,
			interpolation='bilinear')
		self._update_wedge_ax.imshow(self.synth, cmap='viridis_r', aspect=0.2,
			interpolation='bilinear')
		self._update_wedge_ax.imshow(wb.mask_rc(self.refCoef), cmap='Greys',
			aspect=0.2)
	...
	def export_figures(self):
		ricker = QFileDialog.getSaveFileName(self, 'Export Ricker Plot', os.getenv('HOME'),
			'Image Files (*.png *.jpeg *.tiff)')
		self.rickerBox.figure.savefig(ricker[0], dpi=100*10)
		model = QFileDialog.getSaveFileName(self, 'Export Wedge Model', os.getenv('HOME'),
			'Image Files (*.png *.jpeg *.tiff)')
		self.modelBox.figure.savefig(model[0], dpi=100*10)
		amp = QFileDialog.getSaveFileName(self, 'Export Amplitude Plot', os.getenv('HOME'),
			'Image Files (*.png *.jpeg *.tiff)')
